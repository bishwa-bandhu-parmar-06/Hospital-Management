...
const PendingHospitalsAppointments = () => {
  ...
  useEffect(() => {
    const fetchPendingAppointments = async () => {
      try {
        ...
        const response = await axios.get(`${backendUri}/appointment/get-appointments?status=pending&type=hospital`, {
          headers: {
  ...
  const handleCancelAppointment = async (appointmentId) => {
    try {
      const token = localStorage.getItem('hospitalToken');
      await axios.put(`${backendUri}/appointment/${appointmentId}/cancel`, {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Pending Hospital Appointments</h1>
          ...
          <table className="min-w-full bg-white border border-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sm:px-6">Notes</th>
...
export default PendingHospitalsAppointments;